...
def employee():
    ...
    for att in attendance:
        date_obj = datetime.strptime(att['date'], '%Y-%m-%d')  # assuming the date string is in YYYY-MM-DD format
        if date_obj.year == today.year and date_obj.month == today.month and att['status'] == 'absent':
            total_leaves += 1

    return render_template('employee.html', user=session['user'], cal=cal, attendance=attendance, leave_requests=leave_requests, today=today, total_leaves=total_leaves)
...
def checkin():
    ...
    if existing_record:
        if existing_record[2] is not None:
            flash('You have already checked in for today', 'error')
            return redirect(url_for('employee'))
    else:
